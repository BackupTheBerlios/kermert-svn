...
/**
 * Fonction de récupération des informations EXIF d'une image passée en paramètre.
 *
 * @param: le chemin qualifié vers l'image sur le FS
 * @return: une sérialisation SAJAX des données
 */
function ImageInfos($image)
{
     $res = GDWrapper::ImageInfos(km_appdir.km_imagesdir.$image);
     if(!$res)
          return(sajax_serialize("status","KO","libelle","Impossible d'extraire les données EXIF"));
     else
...
sajax_init();
sajax_export("ImageInfos","generateThumb","updateStatus");
