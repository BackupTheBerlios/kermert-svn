...
class wiki2xhtml
{
	...
	function __putMacro($id)
	{
		...
		{
			$content = ltrim($this->macros[$id]);
			
			# Quel type de macro ?
			if (preg_match('/^#!(([a-zA-Z]+).*?$)(.*)\z/ms',$content,$m))
			{
				if (isset($this->functions['macro:'.$m[2]]))
				{
					return call_user_func($this->functions['macro:'.$m[2]],$m[3],$m[1]);
				}
			}
